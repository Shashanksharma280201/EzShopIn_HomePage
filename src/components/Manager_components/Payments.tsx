  ...
  const DeleteItem = async (itemId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        console.error(
          `Error deleting item with ID ${itemId}: ${data.error || response.statusText}`
        );
      }
    } catch (error: any) {
      console.error("Network or API error occurred:", error.message || error);
      console.log(`Failed to delete item with ID ${itemId}. Please try again.`);
    }
  };
